...
namespace eval amritapuri {

    # time (in secs) of penalty for wrong submissions
    variable Penalty		[ expr 20 * 60 ]	;# Default Penalty
    variable ErrorPenalty	;# Special Penalties for some error types

    array set ErrorPenalty [ list 					\
			"Presentation Error" 	[ expr 10 * 60 ]  	\
			    ]
...
proc amritapuri::subjective {} {
    return 0
}

## Sort teams by rank
proc amritapuri::order {teams problems subs} {
    variable Penalty
    variable ErrorPenalty
	...
	if { [ string compare $Classify $accepted ] == 0 } {
	    ...
	    if [ info exists ErrorPenalty($Classify) ] {
		incr penal($Team,$Problem) $ErrorPenalty($Classify)
	    } else {
    ...
    return [ lsort -command ::amritapuri::cmp_pont $teams ]
}

## Assign ranks to teams
proc amritapuri::rank {contest list} {
    variable probs 
...
proc amritapuri::cell {team problem} {
    variable durat
    variable attpt 
